...
const Societes = () => {
  ...
  const [currentSociete, setCurrentSociete] = useState({ nom: '', assuranceId: '', address: '' });
  const [assurancesData, setAssurancesData] = useState([]); // pour récupérer et afficher les assurances
  const [societesData, setSocietesData] = useState([]);

  // Charger les sociétés à partir de l'API
  const loadSociete = async () => {
    try {
      const data = await Apisociete.getSocietes();  // Appel API pour récupérer les sociétés
  ...
  const loadAssurances = async () => {
    try {
      const data = await Apiassurances.getAssurances();  // Appel API pour récupérer les assurances
  ...
  useEffect(() => {
    loadSociete();
    loadAssurances();  // Charger les données lorsque le composant est monté
  }, []);
  ...
  const openModal = (societe = null) => {
    if (societe) {
      ...
      setCurrentSociete({ nom: '', assuranceId: '', address: '' });
      setIsEditing(false);
    }
    setIsModalOpen(true);
  };
  ...
  const handleSave = async () => {
    if (isEditing) {
      ...
      try {
        ...
        console.error('Erreur lors de la sauvegarde de la société:', error);
        alert('Une erreur est survenue lors de l\'ajout de la société');
      }
    }
    closeModal();
  };
            ...
            {filteredSocietes.map((societe) => (
              <tr key={societe.id} className="border-b hover:bg-gray-50">
                ...
                <td className="py-3 px-4 text-sm text-gray-600">
                  {/* Affichage de l'assurance associée */}
                  {assurancesData && assurancesData.find((assurance) => assurance.id === societe.assuranceId)
                    ? assurancesData.find((assurance) => assurance.id === societe.assuranceId).nom
        ...
        <div className="fixed inset-0 bg-gray-500 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-96">
            ...
            <div className="mb-4">
              ...
              <select
                className="w-full p-2 border rounded-md"
                value={currentSociete.assuranceId}
                onChange={(e) => setCurrentSociete({ ...currentSociete, assuranceId: e.target.value })}
              >
