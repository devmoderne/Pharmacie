...
const Detailstickets = () => {
  ...
  const [venteCree, setVenteCree] = useState(false); // pour savoir si vente existe déjà
  ...
  const handleAddLigne = async () => {
   ...
   const detailsObj = {
    ...
    codeTicket: ticketCode,        // ici on garde le code du ticket
  ...
  // Si le backend renvoie un message JSON
...
const handleGenerateTicket = async () => {
  try {
    const response = await detailsVenteService.finaliserTicket(ticketCode);

    // ✅ Message dynamique (si backend renvoie un message, sinon texte par défaut)
    const message = response.data?.message || `✅ Ticket ${ticketCode} finalisé avec succès !`;
    alert(message);
      ...
      <div className="md:col-span-2 bg-white rounded-2xl p-4 shadow">
        {ticketCode && (
          <div className="mb-4 p-3 rounded-xl bg-blue-50 border border-blue-300 text-blue-700 text-center font-semibold shadow-sm">
            <span className="text-gray-600 mr-2">🧾 Code Ticket :</span> {ticketCode}
          </div>
        )}
